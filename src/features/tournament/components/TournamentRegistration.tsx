 ...
 *
 * A component for managing tournament registration (feature-scoped copy)
 *
 * Usage:
...
export type RegisteredSquadStatus =
  | 'pending'
  | 'approved'
  | 'rejected'
  | 'checked_in'
...
export const TournamentRegistration: React.FC<TournamentRegistrationProps> = ({
  ...
  const getStatusLabel = (status: RegisteredSquadStatus): string => {
    switch (status) {
      case 'pending':
        return '⏳ Pending';
      case 'approved':
  ...
  const approvedRegistrations = registeredSquads.filter(
  ...
  return (
    <View style={containerStyle} testID={testID}>
      ...
      <View style={progressContainerStyle}>
        <ProgressBar
          progress={maxSquads > 0 ? registeredSquads.length / maxSquads : 0}
          height={6}
