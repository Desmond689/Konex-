...
class AuthInterceptor {
  ...
  private setupAuthListener(): void {
    // Try to register immediately if possible, otherwise defer until client is ready
    try {
      const maybeAuth: any = (supabase as any)?.auth;
      if (maybeAuth && typeof maybeAuth.onAuthStateChange === 'function') {
        maybeAuth.onAuthStateChange((event: string, session: any) => {
      ...
      logger.debug('AuthInterceptor: auth not available yet, deferring listener registration');
    }

    // Deferred registration
    try {
      // Import getSupabaseClient lazily to avoid any chance of circular require problems
      // when this module is imported very early.
      const { getSupabaseClient } = require('../client/supabase.client');
      getSupabaseClient()
        .then((client: any) => {
        ...
        .catch((error: any) => {
          logger.error('❌ Failed to register deferred auth listener in AuthInterceptor', { error });
        });
    } catch (error) {
      logger.error('❌ Unexpected error while setting up auth listener', { error });
    }
  }
